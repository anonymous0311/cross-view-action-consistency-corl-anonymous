"""Shared LIBERO policy rollout utilities for camera evaluation."""
...
def ensure_libero_config(libero_config_path: str | None = None) -> pathlib.Path:
    ...
    from afcv.evaluation.libero_plus_eval import ensure_wand_compat

    ensure_wand_compat()
    repo_root = pathlib.Path(__file__).resolve().parents[3]
    source_root = pathlib.Path(os.environ.get("LIBERO_ROOT", str(repo_root / "third_party/libero"))).resolve()
    if not source_root.exists():
        raise FileNotFoundError("Install the simulator with scripts/setup_simulator.sh or set LIBERO_ROOT")
    ...
    current_text = cfg_path.read_text(encoding="utf-8") if cfg_path.exists() else None
    if current_text != cfg_text:
        tmp_path = cfg_dir / f".config.{os.getpid()}.tmp"
        tmp_path.write_text(cfg_text, encoding="utf-8")
...
def get_task_suite(task_suite_name: str) -> Any:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            f"LIBERO import failed. Run scripts/setup_simulator.sh (missing module: {exc.name})."
        ) from exc
...
def create_libero_env(task: Any, *, resolution: int = 256, seed: int = 7) -> tuple[Any, str]:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            ...
            "Run scripts/setup_simulator.sh "
...
def build_policy_observation(
    ...
    *,
    ...
    # OpenPI's official LIBERO data uses "both". Locally re-rendered paired
    # images were saved with only flipud, so keep this configurable.
    # wrist_flip_mode=None keeps the legacy coupled behavior (wrist follows
    # image_flip_mode); an explicit mode decouples the wrist orientation for
    # datasets that store the wrist column in raw HDF5 orientation.
