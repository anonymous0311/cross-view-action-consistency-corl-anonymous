    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            "image_mask": {
                ...
                if has_wrist_image or self.model_type == _model.ModelType.PI0_FAST
                else np.False_,
...
class LiberoPairInputs(transforms.DataTransformFn):
    """LIBERO same-state pair inputs for paired cross-view consistency.
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            # This remains pair-level. It controls only the CV reduction and
            # must never be confused with either view's image-validity mask.
            "pair_mask": np.asarray(data.get("pair_mask", np.True_), dtype=np.bool_),
        }
