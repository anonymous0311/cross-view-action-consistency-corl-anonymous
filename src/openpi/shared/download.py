...
def maybe_download(url: str, *, force_download: bool = False, **kwargs) -> pathlib.Path:
    ...
    if local_path.exists():
        # Recover from old cache layouts where a file-like URL was accidentally cached as a directory.
        if local_path.is_dir() and pathlib.Path(parsed.path).suffix:
            invalidate_cache = True
        elif force_download or _should_invalidate_cache(cache_dir, local_path):
            invalidate_cache = True
        else:
            return local_path
...
def _download_gsutil(url: str, local_path: pathlib.Path, **kwargs) -> None:
    ...
    # Try file-style copy first (works for single objects like .json files).
    local_path.parent.mkdir(parents=True, exist_ok=True)
    file_copy = subprocess.run(
        ["gsutil", "cp", url, str(local_path)],
        check=False,
    )
    if file_copy.returncode == 0:
        return

    # Fallback to directory-style recursive copy.
    local_path.mkdir(parents=True, exist_ok=True)
    subprocess.run(
        ["gsutil", "-m", "cp", "-r", f"{url.rstrip('/')}/*", str(local_path)],
        check=True,
    )
