...
Usage:
  python scripts/data/download_libero_hdf5_original.py \
    --download-dir data/libero_original \
...
def download_and_extract(url: str, download_dir: Path, dataset_name: str) -> Path:
    ...
    if zip_path.exists():
        ...
        with requests.get(url, stream=True, allow_redirects=True, timeout=30) as resp:
            ...
            with (
                open(zip_path.with_suffix(".zip.partial"), "wb") as fout,
                tqdm(
                    total=total,
                    unit="B",
                    unit_scale=True,
        ...
        zip_path.with_suffix(".zip.partial").replace(zip_path)
...
def check_dataset(download_dir: Path, datasets: list[str]) -> bool:
    """Verify expected HDF5 counts for each suite (mirrors check_libero_dataset)."""
    print("\n── Dataset integrity check ─────────────────────────────────────────")
    all_ok = True
    suites = [suite for name in datasets for suite in (["libero_10", "libero_90"] if name == "libero_100" else [name])]
    for suite in suites:
...
def main() -> None:
    ...
    parser.add_argument(
        "--download-dir",
        type=Path,
        default=Path("data/libero_original"),
        help="Destination directory (default: data/libero_original)",
    )
    parser.add_argument(
        "--datasets",
    ...
    to_download = list(DATASET_LINKS.keys()) if args.datasets == "all" else [args.datasets]
    ...
    if not args.skip_check:
        ok = check_dataset(download_dir, to_download)
        if ok:
