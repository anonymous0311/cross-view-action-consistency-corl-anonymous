#!/usr/bin/env python3
"""CPU smoke for Phase B multi-sample cross-view loss variants."""
...
class _MultiSampleModel(_SmokeV4Model):
...
class _ActionBiasedBilateralMultiSampleModel(_BilateralMultiSampleModel):
    cv_time_distribution = v4_cv.CV_TIME_DISTRIBUTION_BETA_2P0_3P0
...
def _time_distribution_checks() -> dict:
    sample_shape = (32_768,)
    legacy = v4_cv._sample_cv_time(
        jax.random.key(10),
        v4_cv.CV_TIME_DISTRIBUTION_LEGACY,
        sample_shape,
    )
    action_biased = v4_cv._sample_cv_time(
        jax.random.key(11),
        v4_cv.CV_TIME_DISTRIBUTION_BETA_2P0_3P0,
        sample_shape,
    )
    legacy_mean = float(jnp.mean(legacy))
    action_biased_mean = float(jnp.mean(action_biased))
    return {
        "legacy_mean": legacy_mean,
        "action_biased_mean": action_biased_mean,
        "action_biased_mean_below_legacy": action_biased_mean < legacy_mean,
        "action_biased_mean_in_expected_band": 0.35 <= action_biased_mean <= 0.45,
    }


def main() -> None:
    out_dir = Path("results/v4_plus")
    out_dir.mkdir(parents=True, exist_ok=True)

    legacy = _run_pair_loss(_SmokeV4Model(), batch_size=4)
    ...
    lines = [
        ...
        f"- legacy symmetric loss works: `{legacy['loss_fm_finite'] and legacy['loss_cv_finite']}`",
        ...
        f"- legacy time mean: `{time_checks['legacy_mean']:.4f}`",
        f"- action-biased time mean: `{time_checks['action_biased_mean']:.4f}`",
        f"- action-biased time mean below legacy: `{time_checks['action_biased_mean_below_legacy']}`",
