#!/usr/bin/env bash
# Phase 0A.3 — Merge per-shard JSONL manifests into final combined files.
#
# Run this AFTER all 6 GPU shards have completed.
#
# Usage:
#   cd .
#   bash scripts/v4/phase0A/merge_render_shards.sh
#
# Produces:
#   results/v4_pair_audit/libero_pair_manifest_train.jsonl     (matched pairs)
#   results/v4_pair_audit/libero_pair_manifest_val.jsonl
#   results/v4_pair_audit/libero_wrong_pair_manifest_train.jsonl
#   results/v4_pair_audit/libero_wrong_pair_manifest_val.jsonl

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RESULTS="${REPO}/results/v4_pair_audit"
N_SHARDS=6
...
echo "============================================================"
echo ""
echo "Done. Next step: Phase 0A.5 — LeRobot exporter."
