...
Usage:
  python scripts/v4/phase0A/audit_libero_hdf5_original.py \
...
The script writes:
  results/v4_pair_audit/phase0a_libero_hdf5_audit.md
  results/v4_pair_audit/phase0a_suite_inventory.json
"""
...
# Allow importing phase0a_common whether run from repo root or scripts/v4/phase0A
_HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(_HERE))
from phase0a_common import DEFAULT_RESULTS_DIR, ensure_dir, now_iso, rel_to_repo, write_json
...
def build_markdown(inventory: list[dict], decision: str, generated_at: str) -> str:
    lines = [
        "# Phase 0A: Original LIBERO HDF5 Audit",
        "",
    ...
    decision_text = {
        "LIBERO_RERENDER_PRIMARY": (
            ...
            "Same-state multi-camera rerender is feasible. This is the primary Phase 0A pair source."
        ),
        ...
        "LIBERO_HDF5_UNAVAILABLE": (
            ...
            "Do not start Phase 0B; obtain the original LIBERO HDF5 archive first."
        ),
    ...
    lines += [
        "## Phase 0A Consequence",
        "",
        f"- Global decision: `{decision}`",
        "- `LIBERO_RERENDER_PRIMARY`: proceed with `scripts/v4/phase0A/render_libero_multiview_states.py`.",
        ...
        "- Do not use approximate same-language or same-task pairs in the v4 primary route.",
        "",
...
def main() -> None:
    ...
    if not inventory:
        ...
        print("Example:")
        print("  python scripts/v4/phase0A/audit_libero_hdf5_original.py \\")
    ...
    write_json(output_dir / "phase0a_suite_inventory.json", json_payload)
    print(f"Wrote {output_dir / 'phase0a_suite_inventory.json'}")

    md_text = build_markdown(inventory, decision, generated_at)
    (output_dir / "phase0a_libero_hdf5_audit.md").write_text(md_text, encoding="utf-8")
    print(f"Wrote {output_dir / 'phase0a_libero_hdf5_audit.md'}")
