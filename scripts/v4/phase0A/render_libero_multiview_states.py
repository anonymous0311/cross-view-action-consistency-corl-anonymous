"""Phase 0A.3 — Same-state multiview rerender for LIBERO HDF5 demonstrations.
...
Outputs:
  ...
  results/v4_pair_audit/libero_pair_manifest_train.jsonl
  results/v4_pair_audit/libero_pair_manifest_val.jsonl
  results/v4_pair_audit/libero_wrong_pair_manifest_{train,val}.jsonl
  results/v4_pair_audit/rerender_reconstruction_report.md
  results/v4_pair_audit/libero_pair_audit_report.md
...
Smoke test (verify state reset works before batch render):
  venv-libero-plus/bin/python scripts/v4/phase0A/render_libero_multiview_states.py \\
...
sys.path.insert(0, str(_HERE))  # phase0a_common

from phase0a_common import (
    DEFAULT_RESULTS_DIR,
    ensure_dir,
    hash_array,
    now_iso,
...
DEFAULT_RESULTS_DIR_= REPO_ROOT / "results" / "v4_pair_audit"
...
# ── Camera sampling pools (from configs/v4_camera_perturbation_spec.yaml) ───────
...
def _rerender_report(
    ...
    lines = [
        "# Phase 0A.3 — Rerender Reconstruction Report",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 0A.3: same-state multiview LIBERO rerender.",
