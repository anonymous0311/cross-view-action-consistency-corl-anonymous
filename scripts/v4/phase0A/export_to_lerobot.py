"""Phase 0A.5 — Export rendered LIBERO multiview pairs to LeRobot-compatible format.

Reads the merged pair manifests from Phase 0A.3 and builds a LeRobot dataset
...
After export, verify with:
    venv-libero-plus/bin/python scripts/v4/phase0A/export_to_lerobot.py --verify-only \\
        --output-dir data/rendered_libero_multiview_lerobot
"""
...
def main() -> None:
    ...
    parser.add_argument("--manifest-train", type=Path,
                        default=REPO_ROOT / "results/v4_pair_audit/libero_pair_manifest_train.jsonl")
    parser.add_argument("--manifest-val",   type=Path,
                        default=REPO_ROOT / "results/v4_pair_audit/libero_pair_manifest_val.jsonl")
    ...
    print(f"\nNext: verify with --verify-only, then write LeRobotV4PairDataConfig in config.py")
