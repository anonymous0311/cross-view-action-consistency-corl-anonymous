#!/usr/bin/env bash
# Phase 0A.3 — Full LIBERO rerender across 6 GPUs in parallel.
#
# Renders ALL frames (100% sampling) from 4 eval suites once.
# This single dataset is used for both Phase 0B screening and confirmation.
#
# Expected output:
...
# Usage:
#   cd .
#   bash scripts/v4/phase0A/launch_render_6gpu.sh
#
# Monitor progress:
...
SCRIPT="${REPO}/scripts/v4/phase0A/render_libero_multiview_states.py"
LIBERO_ROOT="data/libero_hdf5_original"
OUTPUT_DIR="${REPO}/data/rendered_libero_multiview"
RESULTS_DIR="${REPO}/results/v4_pair_audit"
...
echo "  bash scripts/v4/phase0A/merge_render_shards.sh"
