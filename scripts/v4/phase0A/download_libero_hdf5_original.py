...
Usage:
  python scripts/v4/phase0A/download_libero_hdf5_original.py \
...
def main() -> None:
    ...
    if not args.skip_check:
        ...
        if ok:
            ...
            print(f"\nRe-run audit with:")
            print(f"  .venv/bin/python scripts/v4/phase0A/audit_libero_hdf5_original.py \\")
            print(f"    --libero-root {download_dir} \\")
            print(f"    --suite-dirs libero_spatial libero_object libero_goal libero_90 libero_10 \\")
            print(f"    --output-dir results/v4_pair_audit")
        else:
