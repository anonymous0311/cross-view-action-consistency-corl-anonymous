...
class Pi0(_model.BaseModel):
    def __init__(self, config: pi0_config.Pi0Config, rngs: nnx.Rngs):
        ...
        self.canonical_token_dropout = config.canonical_token_dropout
        ...
        llm = nnx_bridge.ToNNX(
            _gemma.Module(
                ...
                adarms=config.pi05,
                canonical_dim=config.canonical_dim,
            )
        )
    ...
    def predict_flow(
        ...
        *,
        ...
        suffix_tokens, suffix_mask, suffix_ar_mask, adarms_cond = self.embed_suffix(observation, noisy_actions, time)
        ...
        (prefix_out, suffix_out), _ = self.PaliGemma.llm(
            ...
            adarms_cond=[None, adarms_cond], canonical_tokens=canonical_tokens,
        )
        del prefix_out
        return self.action_out_proj(suffix_out[:, -self.action_horizon :])

    def capture_prefix_intermediates(
        self,
        observation: _model.Observation,
        *,
        ...
        """Capture contextualized prefix hidden states for routing audits.

        This is an audit-only forward. It mirrors the prefix pass used by
        ``sample_actions`` and ``predict_flow`` but returns selected hidden
        states for the image/language prefix stream. It does not change the
        ...
        prefix_tokens, prefix_mask, prefix_ar_mask = self.embed_prefix(observation)
        n_language_tokens = 0 if observation.tokenized_prompt is None else observation.tokenized_prompt.shape[-1]
        n_image_tokens = prefix_tokens.shape[1] - n_language_tokens
        if zero_image_tokens:
            prefix_tokens = prefix_tokens.at[:, :n_image_tokens, :].set(jnp.zeros_like(prefix_tokens[:, :n_image_tokens, :]))
        prefix_attn_mask = make_attn_mask(prefix_mask, prefix_ar_mask)
        positions = jnp.cumsum(prefix_mask, axis=1) - 1
        if layer_indices is None:
            layer_indices = tuple(range(self.paligemma_depth))

        (prefix_out, _), _kv_cache, layer_prefix_hidden = self.PaliGemma.llm(
            [prefix_tokens, None],
            mask=prefix_attn_mask,
            positions=positions,
            method="capture_intermediates",
            capture_layers=tuple(layer_indices),
        )
        return {
            "raw_prefix": prefix_tokens,
            "prefix_out": prefix_out,
            "layer_prefix_hidden": layer_prefix_hidden,
            "prefix_mask": prefix_mask,
            "n_image_tokens": n_image_tokens,
            "n_language_tokens": n_language_tokens,
        }
