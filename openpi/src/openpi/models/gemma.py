...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True, canonical_tokens=None):  # noqa: FBT002
        ...
        if self.canonical_dim > 0 and canonical_tokens is not None and xs[1] is not None:
            ...
            N_heads = self.configs[1].num_heads
            ...
            # Standard pre-LN (NOT adaRMS) so that gate=None and _gated_residual = x2 + out_can.
            # adaRMS would zero-init the gate; combined with zero-init can_out this creates a
            ...
            q = einops.rearrange(q, "B T (N H) -> B T N H", N=N_heads)
            ...
            out_can = nn.Dense(
                ...
                name="can_out"
            )(encoded)  # [B, T_action, D_action]

            xs = [xs[0], _gated_residual(x2, out_can, gate)]
            xs = sharding.activation_sharding_constraint(xs)
...
class BlockWithPrefixCapture(Block):
    """Transformer block variant that scans prefix hidden states for audits.

    This intentionally reuses the exact submodule names from ``Block`` so the
    scanned parameters live under the same ``layers/...`` path. The normal
    model forward path discards the captured hidden states.
    """
...
class Module(nn.Module):
    ...
    adarms: bool = False
    canonical_dim: int = 0  # 0 = disabled; propagated to all Block layers
    ...
    def setup(self):
        ...
        in_axes_base = (
            0,           # kv_cache: scanned (per-layer state)
    ...
    def __call__(
        ...
        *,
        ...
        canonical_tokens: at.Float[at.Array, "b _n _d"] | None = None,
        ...
        if self.canonical_dim > 0:
            embedded, kv_cache = self.layers(embedded, kv_cache, positions, mask, adarms_cond, deterministic, canonical_tokens)
        else:
    ...
    def capture_intermediates(
        ...
        *,
    ...
    ) -> tuple[Sequence[at.Float[at.Array, "b _t _d"] | None], KVCache, at.Float[at.Array, "_l b _t _d"]]:
        """Runs the Gemma trunk and returns selected per-layer prefix hidden states.

        This audit-only method gathers the first expert's hidden state after
        each selected block. For pi0/pi0.5 prefix-only audits, the first expert
        is the contextualized image/language prefix stream.
        """
        embedded = jax.tree.map(lambda e: e.astype(self.embed_dtype), embedded)
        mask = jnp.asarray(mask)[:, None, :, :]
        if adarms_cond is None:
        ...
        if capture_layers is None:
            capture_layers = tuple(range(self.configs[0].depth))

        block_cls = nn.remat(
            BlockWithPrefixCapture,
            prevent_cse=False,
            static_argnums=(5,),  # 0=self, 6=deterministic
            policy=jax.checkpoint_policies.nothing_saveable,
        )
        ...
        capture_layers_module = nn.scan(
            ...
            parent=self.scope.push("layers", reuse=True),
            configs=self.configs,
            canonical_dim=self.canonical_dim,
            dropout=self.dropout,
            dropout_bdims=self.dropout_bdims,
        ...
        if self.canonical_dim > 0:
            embedded, (kv_cache, prefix_hidden_by_layer) = capture_layers_module(
                embedded,
                kv_cache,
                positions,
                mask,
        ...
        assert all(e.dtype == jnp.dtype(self.embed_dtype) for e in embedded if e is not None)
        capture_layer_indices = jnp.asarray(capture_layers, dtype=jnp.int32)
        selected_prefix_hidden = jnp.take(prefix_hidden_by_layer, capture_layer_indices, axis=0)
        return [
            f(e, a)[0] if e is not None else e for f, e, a in zip(self.final_norms, embedded, adarms_cond, strict=True)
        ], kv_cache, selected_prefix_hidden
    ...
    def init(self, use_adarms: Sequence[bool]):
        ...
        if self.canonical_dim > 0:
            canonical_tokens = jnp.zeros((1, 1, self.canonical_dim), dtype=self.embed_dtype)
        self(
