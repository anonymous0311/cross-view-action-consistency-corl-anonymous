...
from openpi.models import cross_view_action_consistency_config
...
def _sample_cv_time(
    ...
    if distribution == CV_TIME_DISTRIBUTION_BETA_1P5_1:
...
def preprocess_pair_observation(
    ...
    *,
    ...
    ``current/current`` intentionally delegates to the standard OpenPI path:
...
class Pi0CrossViewActionConsistency(pi0.Pi0):
    """Vanilla pi0 with cross-view action-flow consistency loss."""

    def __init__(self, config: cross_view_action_consistency_config.Pi0CrossViewActionConsistencyConfig, rngs):
    ...
    def _pair_loss_components(
        ...
        *,
        ...
        if actions.ndim != 4 or actions.shape[1] != 2:
            raise ValueError(
                "Pi0CrossViewActionConsistency expects paired actions with shape [B, 2, action_horizon, action_dim], "
                f"got {actions.shape}"
            )
        ...
        if cv_loss_mode == CV_LOSS_MODE_SYMMETRIC:
            ...
            time = _sample_cv_time(
                time_rng,
                getattr(self, "cv_time_distribution", CV_TIME_DISTRIBUTION_BETA_1P5_1),
                batch_shape,
            )
            ...
            time_distribution = getattr(self, "cv_time_distribution", CV_TIME_DISTRIBUTION_BETA_1P5_1)
    ...
    def compute_train_metrics(
        ...
        *,
        ...
        return {
            "cross_view/loss_fm": jnp.mean(components["loss_fm"]),
            "cross_view/loss_fm_nominal": jnp.mean(components["loss_fm_nominal"]),
            "cross_view/loss_fm_perturbed": jnp.mean(components["loss_fm_perturbed"]),
            "cross_view/loss_cv": jnp.mean(components["loss_cv"]),
            "cross_view/lambda_cv": components["lambda_cv"],
            "cross_view/flow_disagreement": jnp.mean(components["flow_disagreement"]),
            "cross_view/action_flow_norm": jnp.mean(components["action_norm"]),
            "cross_view/clean_wrong_cv_active": components["clean_wrong_cv_active"],
            "cross_view/multi_sample_asymmetric_cv_active": components["multi_sample_asymmetric_cv_active"],
            "cross_view/cv_num_samples": components["cv_num_samples"],
        }
