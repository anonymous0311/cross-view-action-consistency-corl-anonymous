...
class _FakeV4Model:
    cv_action_dim = 7
    cv_pair_mode = v4_cv.CV_PAIR_MODE_MATCHED
    cv_loss_mode = v4_cv.CV_LOSS_MODE_SYMMETRIC
    cv_num_samples = 1
    cv_stopgrad_anchor = True
    cv_time_distribution = v4_cv.CV_TIME_DISTRIBUTION_LEGACY
    cv_eps_shared_across_views = True
    cv_average_over_samples = True
    pair_spatial_aug_mode = v4_cv.PAIR_SPATIAL_AUG_MODE_CURRENT
    pair_photometric_aug_mode = v4_cv.PAIR_PHOTOMETRIC_AUG_MODE_CURRENT

    def _lambda_cv(self, step):
        del step
        return jnp.asarray(0.05, dtype=jnp.float32)

    _flatten_pair_observation = v4_cv.Pi0V4CV._flatten_pair_observation
...
class _FakeCleanWrongV4Model(_FakeV4Model):
    cv_pair_mode = v4_cv.CV_PAIR_MODE_CLEAN_WRONG_BATCH_DERANGEMENT
...
class _FakeMultiSampleAsymmetricV4Model(_FakeV4Model):
    cv_loss_mode = v4_cv.CV_LOSS_MODE_MULTI_SAMPLE_ASYMMETRIC
    cv_num_samples = 2
...
class _FakeCleanWrongMultiSampleAsymmetricV4Model(_FakeMultiSampleAsymmetricV4Model):
    cv_pair_mode = v4_cv.CV_PAIR_MODE_CLEAN_WRONG_BATCH_DERANGEMENT
...
def test_v4_lambda_cv_linear_ramp():
    np.testing.assert_allclose(
        v4_cv._linear_ramp_lambda(0.1, 0, total_steps=100, start_fraction=0.1, end_fraction=0.3),
        0.0,
    )
    np.testing.assert_allclose(
        v4_cv._linear_ramp_lambda(0.1, 20, total_steps=100, start_fraction=0.1, end_fraction=0.3),
        0.05,
        rtol=1e-6,
    )
    np.testing.assert_allclose(
        v4_cv._linear_ramp_lambda(0.1, 30, total_steps=100, start_fraction=0.1, end_fraction=0.3),
        0.1,
        rtol=1e-6,
    )
...
def test_sample_cv_time_supports_beta_1p0_1p5_distribution():
    time = v4_cv._sample_cv_time(
        jax.random.key(0),
        v4_cv.CV_TIME_DISTRIBUTION_BETA_1P0_1P5,
        (4096,),
    )
...
def test_v4_pair_loss_shares_noise_and_time_within_pair():
    model = _FakeV4Model()
    observation = _make_pair_observation(batch_size=2)
    actions = jnp.ones((2, 2, 3, 8), dtype=jnp.float32)

    components = v4_cv.Pi0V4CV._pair_loss_components(
        model,
        jax.random.key(0),
        observation,
        actions,
...
def test_v4_pair_loss_accepts_base_only_images():
    model = _FakeV4Model()
...
def test_multi_sample_asymmetric_pair_loss_k2_uses_shared_noise_and_asymmetric_cv():
    model = _FakeMultiSampleAsymmetricV4Model()
    observation = _make_pair_observation(batch_size=2, include_wrist=False)
    actions = jnp.ones((2, 2, 3, 8), dtype=jnp.float32)
...
def test_multi_sample_asymmetric_cv_stopgrad_blocks_nominal_cv_gradient():
    ...
    def loss_with_stopgrad(v):
        return jnp.mean(
            v4_cv._multi_sample_asymmetric_cv_loss(
                v,
                cv_dim=7,
                anchor_indices=None,
...
def test_pair_preprocess_none_spatial_none_photometric_preserves_pair_axis_then_flattens():
    ...
    out = v4_cv.preprocess_pair_observation(
        jax.random.key(0),
        observation,
        train=True,
        image_keys=("base_0_rgb",),
        pair_spatial_aug_mode=v4_cv.PAIR_SPATIAL_AUG_MODE_NONE,
        pair_photometric_aug_mode=v4_cv.PAIR_PHOTOMETRIC_AUG_MODE_NONE,
    )
...
def test_pair_preprocess_shared_spatial_uses_same_transform_for_both_slots():
    ...
    out = v4_cv.preprocess_pair_observation(
        ...
        train=True,
        image_keys=("base_0_rgb",),
        pair_spatial_aug_mode=v4_cv.PAIR_SPATIAL_AUG_MODE_SHARED,
        pair_photometric_aug_mode=v4_cv.PAIR_PHOTOMETRIC_AUG_MODE_NONE,
    )
...
def test_pair_preprocess_shared_photometric_uses_same_transform_for_both_slots():
    ...
    out = v4_cv.preprocess_pair_observation(
        ...
        image_keys=("base_0_rgb",),
        pair_spatial_aug_mode=v4_cv.PAIR_SPATIAL_AUG_MODE_NONE,
        pair_photometric_aug_mode=v4_cv.PAIR_PHOTOMETRIC_AUG_MODE_SHARED,
    )
...
def test_v4_pair_loss_fixedaug_path_runs():
    class _FakeFixedAugV4Model(_FakeV4Model):
        pair_spatial_aug_mode = v4_cv.PAIR_SPATIAL_AUG_MODE_NONE
        pair_photometric_aug_mode = v4_cv.PAIR_PHOTOMETRIC_AUG_MODE_INDEPENDENT

    model = _FakeFixedAugV4Model()
    observation = _make_pair_observation(batch_size=2, include_wrist=False)
    actions = jnp.ones((2, 2, 3, 8), dtype=jnp.float32)
...
def test_clean_wrong_pairing_deranges_second_slot_with_own_labels():
    ...
    wrong_observation, wrong_actions = v4_cv._apply_clean_wrong_pairing(observation, actions, perm)
...
def test_clean_wrong_derangement_has_no_fixed_points():
    batch_size = 16
    perm = np.asarray(v4_cv._random_derangement_permutation(jax.random.key(0), batch_size))
...
def test_clean_wrong_multi_sample_asymmetric_deranges_only_cv_anchor():
    model = _FakeCleanWrongMultiSampleAsymmetricV4Model()
    observation = _make_pair_observation(batch_size=4, include_wrist=False)
    observation = dataclasses.replace(
        observation,
        state=jnp.arange(4 * 2 * 8, dtype=jnp.float32).reshape(4, 2, 8),
