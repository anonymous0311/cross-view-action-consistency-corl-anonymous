...
def test_cross_view_single_sample_matches_lambda010_except_pair_aug_and_steps():
    baseline = _config.get_config("pi05_cross_view_lambda010")
    no_spatial = _config.get_config("pi05_cross_view_single_sample")
...
def test_multi_sample_asymmetric_cross_view_config_is_primary_lambda010():
    matched = _config.get_config("pi05_cross_view_multisample_stopgrad")
    clean_wrong = _config.get_config("pi05_cross_view_clean_wrong_multisample_stopgrad")
    ...
    for cfg in (matched, clean_wrong):
        ...
        assert cfg.model.cv_time_distribution == "beta_1p5_1"
    ...
    assert clean_wrong.policy_metadata["matched_reference"] == "pi05_cross_view_multisample_stopgrad"


def test_multi_sample_bilateral_cross_view_only_changes_stopgrad():
    asymmetric = _config.get_config("pi05_cross_view_multisample_stopgrad")
    bilateral = _config.get_config("pi05_cross_view_multisample_bilateral_beta_1p5_1_time")
...
def test_cross_view_action_biased_time_config_only_changes_time_from_bilateral():
    bilateral = _config.get_config("pi05_cross_view_multisample_bilateral_beta_1p5_1_time")
    action_biased = _config.get_config("pi05_cross_view_action_consistency")
    ...
    assert action_biased.policy_metadata["method_variant"] == "cross_view_action_consistency"
    ...
    assert (
        action_biased.policy_metadata["bilateral_beta_1p5_1_time_reference"]
        == "pi05_cross_view_multisample_bilateral_beta_1p5_1_time"
    )
...
def test_cross_view_clean_wrong_only_changes_cv_pairing_from_matched_cross_view():
    matched = _config.get_config("pi05_cross_view_action_consistency")
    clean_wrong = _config.get_config("pi05_cross_view_action_consistency_clean_wrong")
    ...
    assert type(clean_wrong.data).__name__ == "LeRobotCrossViewPairDataConfig"
    assert clean_wrong.data.repo_id == matched.data.repo_id


def test_cross_view_independent_ablation_train_configs_isolate_one_axis():
    baseline = _config.get_config("pi05_cross_view_action_consistency")
    specs = {
    ...
    for name, spec in specs.items():
        ...
        assert type(cfg.data).__name__ == "LeRobotCrossViewPairDataConfig"
        ...
        assert cfg.model.pair_photometric_aug_mode == "independent"
        assert cfg.num_train_steps == 10_000
        assert cfg.policy_metadata["experiment_group"] == "cross_view_action_consistency_ablation"
...
def test_cross_view_independent_ablation_eval_configs_use_single_view_eval_data():
    train_names = [
        "pi05_cross_view_action_consistency_stopgrad",
        "pi05_cross_view_action_consistency_k1",
        "pi05_cross_view_action_consistency_k4",
    ...
    for train_name in train_names:
        ...
        assert type(eval_cfg.data).__name__ == "LiberoCrossViewEvalDataConfig"
        assert type(train.data).__name__ == "LeRobotCrossViewPairDataConfig"
...
def test_multi_sample_asymmetric_eval_config_uses_single_view_eval_data():
    train = _config.get_config("pi05_cross_view_multisample_stopgrad")
    eval_cfg = _config.get_config("pi05_cross_view_multisample_stopgrad_eval")
    ...
    clean_eval = _config.get_config("pi05_cross_view_clean_wrong_multisample_stopgrad_eval")
    assert type(clean_eval.data).__name__ == "LiberoCrossViewEvalDataConfig"
    assert clean_eval.model.cv_pair_mode == "clean_wrong_batch_derangement"
    assert clean_eval.policy_metadata["matched_reference"] == "pi05_cross_view_multisample_stopgrad"


def test_multi_sample_bilateral_eval_config_uses_single_view_eval_data():
    train = _config.get_config("pi05_cross_view_multisample_bilateral_beta_1p5_1_time")
    eval_cfg = _config.get_config("pi05_cross_view_multisample_bilateral_beta_1p5_1_time_eval")
...
def test_cross_view_action_biased_time_eval_config_uses_single_view_eval_data():
    train = _config.get_config("pi05_cross_view_action_consistency")
    eval_cfg = _config.get_config("pi05_cross_view_action_consistency_eval")

    assert type(eval_cfg.data).__name__ == "LiberoCrossViewEvalDataConfig"
    assert type(train.data).__name__ == "LeRobotCrossViewPairDataConfig"
    assert eval_cfg.policy_metadata["eval_only"] is True
    assert eval_cfg.policy_metadata["train_config"] == train.name
    assert eval_cfg.policy_metadata["method_variant"] == "cross_view_action_consistency"
...
def test_cross_view_clean_wrong_eval_config_uses_single_view_eval_data():
    train = _config.get_config("pi05_cross_view_action_consistency_clean_wrong")
    eval_cfg = _config.get_config("pi05_cross_view_action_consistency_clean_wrong_eval")

    assert type(eval_cfg.data).__name__ == "LiberoCrossViewEvalDataConfig"
    assert type(train.data).__name__ == "LeRobotCrossViewPairDataConfig"
    assert eval_cfg.policy_metadata["eval_only"] is True
    assert eval_cfg.policy_metadata["train_config"] == train.name
    assert eval_cfg.policy_metadata["method_variant"] == "clean-wrong-control"
...
def test_realman_task123_cross_view_config_uses_combined_pair_data_and_realman_actions():
    cfg = _config.get_config("pi05_realman_task123_cross_view_action_consistency")

    assert type(cfg.data).__name__ == "LeRobotCrossViewPairDataConfig"
    assert cfg.data.repo_id == "data/real_robot/task123_pair"
    assert cfg.data.assets.assets_dir == "assets/pi05_realman_task123_cross_view_action_consistency"
    ...
    assert cfg.model.pair_photometric_aug_mode == "independent"
    assert cfg.num_train_steps == 10_000
    assert cfg.policy_metadata["method_variant"] == "Task123_CoRL-cross_view_action_consistency"
...
def test_realman_task123_cross_view_eval_uses_single_view_val_split():
    train = _config.get_config("pi05_realman_task123_cross_view_action_consistency")
    eval_cfg = _config.get_config("pi05_realman_task123_cross_view_action_consistency_eval")
...
def test_realman_task123_cross_view_action_expert_only_small_config_is_conservative_short_run():
    base = _config.get_config("pi05_realman_task123_cross_view_action_consistency")
    cfg = _config.get_config("pi05_realman_task123_cross_view_action_consistency_action_expert_only_5k_lr2e5")
    ...
    assert cfg.model.cv_warmup_start_fraction == 0.0
    assert cfg.model.cv_warmup_end_fraction == 0.05
    assert cfg.policy_metadata["method"] == "cross_view_cv_action_expert_only"
...
def test_realman_task123_cross_view_action_expert_only_small_eval_matches_train_model():
    train = _config.get_config("pi05_realman_task123_cross_view_action_consistency_action_expert_only_5k_lr2e5")
    eval_cfg = _config.get_config("pi05_realman_task123_cross_view_action_consistency_action_expert_only_5k_lr2e5_eval")
...
def test_realman_task123_cross_view_eval_h20_only_changes_inference_horizon():
    base_eval = _config.get_config("pi05_realman_task123_cross_view_action_consistency_eval")
    h20 = _config.get_config("pi05_realman_task123_cross_view_action_consistency_eval_h20")
    ...
    assert h20.policy_metadata["train_config"] == "pi05_realman_task123_cross_view_action_consistency"
